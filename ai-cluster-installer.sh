...
uninstall_tool() {
  ...
  sudo rm -f "$USER_HOME/network-switch.sh"
  sudo rm -f "$USER_HOME/enable_dhcp.sh"
  sudo rm -f "$USER_HOME/restore_network.sh"
  sudo rm -rf "$BACKUP_DIR"
  sudo rm -f /etc/netplan/99-temporary-network.yaml /etc/netplan/linshi.network.yaml
